...
namespace Ubpa {
	...
	private:
		...
		/*SampleLightResult SampleLight(IntersectorClosest::Rst intersection, const vecf3& wo, const Cmpt::Light* light, const Cmpt::L2W* l2w, const Cmpt::SObjPtr* ptr,
				std::map<std::pair<int, int>, float> env_light_pdf, std::vector<std::pair<float, float>> env_light_alias_table_p, std::vector< std::pair<std::pair<int, int>, std::pair<int, int>> > env_light_alias_table_idx)  const;*/
		...
		std::vector<std::pair<float, float>> env_light_alias_table_p;
		std::vector< std::pair<std::pair<int, int>, std::pair<int, int>> > env_light_alias_table_idx;
		std::map<std::pair<int, int>, float> env_light_pdf;
